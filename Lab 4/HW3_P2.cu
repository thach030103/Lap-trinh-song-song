        ...
        bool useDevice=false, dim3 blockSize=dim3(1), int nStreams=1)
{
	...
	else // Use device
	{
		...
		int chunkSize = (n + nStreams - 1) / nStreams;  // Kích thước dữ liệu cho mỗi stream
		for (int i = 0; i < nStreams; i++)
		{
			int offset = i * chunkSize;
			int nElems = (offset + chunkSize < n) ? chunkSize : (n - offset);

			CHECK(cudaMemcpyAsync(&d_in1[offset], &in1[offset], nElems * sizeof(int), cudaMemcpyHostToDevice, streams[i]));
			CHECK(cudaMemcpyAsync(&d_in2[offset], &in2[offset], nElems * sizeof(int), cudaMemcpyHostToDevice, streams[i]));

			int gridSize = (nElems - 1) / blockSize.x + 1;
			addVecKernel<<<gridSize, blockSize, 0, streams[i]>>>(&d_in1[offset], &d_in2[offset], nElems, &d_out[offset]);

			CHECK(cudaMemcpyAsync(&out[offset], &d_out[offset], nElems * sizeof(int), cudaMemcpyDeviceToHost, streams[i]));
		}
