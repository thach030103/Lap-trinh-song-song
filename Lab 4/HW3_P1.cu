                               ...
                               uchar3 * outPixels)
{
    ...
    int snemWidth = blockDim.x + 2 * radius; 
    ...
    for (int d_R = -radius; d_R <= radius; ++d_R) {
        for (int d_C = -radius; d_C <= radius; ++d_C) {
            int g_R = min(height - 1,max(0,r + d_R));
            int g_C = min(width - 1,max(0,c + d_C));

            s_inPixels[(s_R + d_R) * snemWidth + (s_C + d_C)] = inPixels[g_R * width + g_C];
        }
    }
    ...
    if (r < height && c < width) {
        ...
        for (int fRow = 0; fRow < filterWidth; fRow++) {
            for (int fCol = 0; fCol < filterWidth; fCol++) {
                float fVal = filter[fRow * filterWidth + fCol];
                int inPixelsR = s_R + fRow - radius;
                int inPixelsC = s_C + fCol - radius;
                
                uchar3 inPixel = s_inPixels[inPixelsR * snemWidth + inPixelsC];
        ...
        uchar3 * outPixels)
{
    ...
    if (r < height && c < width) {
        ...
        for (int fRow = 0; fRow < filterWidth; fRow++) {
            for (int fCol = 0; fCol < filterWidth; fCol++) {
                float fVal = dc_filter[fRow * filterWidth + fCol];
                int inPixelsR = s_R + fRow - radius;
                int inPixelsC = s_C + fCol - radius;
        ...
        bool useDevice=false, dim3 blockSize=dim3(1, 1), int kernelType=1)
{
	...
	else // Use device
	{
		...
		else if (kernelType == 2)
		{
			...
			blurImgKernel2<<<gridSize, blockSize, (blockSize.x + 2 * (filterWidth / 2)) * (blockSize.y + 2 * (filterWidth / 2)) * sizeof(uchar3)>>>(
		...
		else
		{
			...
			blurImgKernel3<<<gridSize, blockSize, (blockSize.x + 2 * (filterWidth / 2)) * (blockSize.y + 2 * (filterWidth / 2)) * sizeof(uchar3)>>>(
